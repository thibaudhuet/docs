...
```python
resp = requests.get(
    "https://api.simcel.io/api/master/v1/customers",
...
resp = requests.get(
    "https://api.simcel.io/api/master/v1/facilities",
    headers={"Authorization": f"Bearer {token}"},
    params={
        "workspaceId": "ws_abc123",
        "type": "DC",
...
```python
resp = requests.patch(
    "https://api.simcel.io/api/master/v1/facilities/DC-PARIS",
    headers={
...
```python
supply = requests.get(
    "https://api.simcel.io/api/insights/v1/supply-performance",
    headers={"Authorization": f"Bearer {token}"},
    params={
        "planId": "plan_9xKz2",
